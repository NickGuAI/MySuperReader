...
// Updated to use AppId and AppKey for Inoreader authentication
export async function fetchNews(): Promise<NewsItem[]> {
  try {
    ...
    // Try a different authentication approach - using Authorization header with Basic auth
    // Convert AppId:AppKey to base64
    const authString = Buffer.from(`${appId}:${appKey}`).toString("base64")

    const response = await fetch(
      "https://www.inoreader.com/reader/api/0/stream/contents/user/-/state/com.google/reading-list?n=20",
      {
        headers: {
    ...
    return convertInoreaderToNewsItems(data)
  } catch (error) {
    console.error("Error fetching from Inoreader API:", error)
...
// Update the convertInoreaderToNewsItems function to handle the actual API response format
function convertInoreaderToNewsItems(data: any): NewsItem[] {
  ...
  return data.items.map((item: any) => {
    ...
    const categories = item.categories
      ? item.categories
          .filter((cat: string) => !cat.includes("user/") && !cat.includes("state/"))
          .map((cat: string) => {
